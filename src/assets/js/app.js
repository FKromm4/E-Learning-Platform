...
// RENDER FUNCTIONS
// ============================================
function renderCourses(courses, container, showFavouriteBtn = true) {
    if (courses.length === 0) {
    ...
    container.innerHTML = courses.map(course => {
        const isFav = isAuthenticated && typeof favouritesService !== 'undefined' && favouritesService.isFavourite(course.id, 'course');
        const favBtn = showFavouriteBtn && isAuthenticated ? `
      <button class="favourite-btn ${isFav ? 'active' : ''}" 
              onclick="toggleFavourite(event, ${course.id}, 'course')" 
              aria-label="${isFav ? 'Αφαίρεση από αγαπημένα' : 'Προσθήκη στα αγαπημένα'}">
        <span class="heart heart-outline">🤍</span>
        <span class="heart heart-filled">❤️</span>
      </button>
    ` : '';
...
function renderBooks(books, container, showFavouriteBtn = true) {
    if (books.length === 0) {
    ...
    container.innerHTML = books.map(book => {
        const isFav = isAuthenticated && typeof favouritesService !== 'undefined' && favouritesService.isFavourite(book.id, 'book');
        const favBtn = showFavouriteBtn && isAuthenticated ? `
      <button class="favourite-btn ${isFav ? 'active' : ''}" 
              onclick="toggleFavourite(event, ${book.id}, 'book')" 
              aria-label="${isFav ? 'Αφαίρεση από αγαπημένα' : 'Προσθήκη στα αγαπημένα'}">
        <span class="heart heart-outline">🤍</span>
        <span class="heart heart-filled">❤️</span>
      </button>
...
/**
 * Handle add to favorites action (from course-details page)
 */
function handleAddToFavorites(event) {
    ...
    const isNowFavourite = favouritesService.toggleFavourite(itemId, itemType);

    if (isNowFavourite) {
        showNotification('Προστέθηκε στα αγαπημένα σας!', 'success');
    } else {
        showNotification('Αφαιρέθηκε από τα αγαπημένα σας.', 'info');
    }
}

/**
 * Toggle favourite state (from card buttons)
 */
function toggleFavourite(event, itemId, itemType) {
    event.preventDefault();
    ...
    btn.setAttribute('aria-label', isNowFavourite ? 'Αφαίρεση από αγαπημένα' : 'Προσθήκη στα αγαπημένα');

    if (isNowFavourite) {
        showNotification('Προστέθηκε στα αγαπημένα!', 'success');
    } else {
        showNotification('Αφαιρέθηκε από τα αγαπημένα.', 'info');
        // If we're on the favourites page, re-render the list
        if (document.body.dataset.page === 'favourites') {
            const activeFilter = document.querySelector('.filter-tab.active')?.dataset.filter || 'all';
            renderFavouritesPage(activeFilter);
        }
    }
}
...
function initFavouritesPage() {
    ...
    filterTabs.forEach(tab => {
        tab.addEventListener('click', function () {
            ...
            renderFavouritesPage(this.dataset.filter);
        });
    });
}

function renderFavouritesPage(filter = 'all') {
    const container = document.getElementById('favourites-list');
    if (!container) return;

    const favourites = favouritesService.getAllFavouriteItems();
    const courses = favourites.courses;
    const books = favourites.books;
    ...
    if (countAll) countAll.textContent = courses.length + books.length;
    if (countCourses) countCourses.textContent = courses.length;
    if (countBooks) countBooks.textContent = books.length;

    // Determine what to show based on filter
    let itemsToShow = [];
    if (filter === 'all' || filter === 'courses') {
        itemsToShow = itemsToShow.concat(courses.map(c => ({ ...c, _type: 'course' })));
    }
    if (filter === 'all' || filter === 'books') {
        itemsToShow = itemsToShow.concat(books.map(b => ({ ...b, _type: 'book' })));
    }
    ...
    if (itemsToShow.length === 0) {
      ...
      <div class="favourites-empty">
        <div class="favourites-empty-icon">💔</div>
        <h3>Δεν έχετε αγαπημένα ακόμα</h3>
        <p>Εξερευνήστε τα μαθήματα και τα βιβλία μας και προσθέστε τα στα αγαπημένα σας για εύκολη πρόσβαση.</p>
        <a href="courses.html" class="btn btn-primary">Εξερεύνηση Μαθημάτων</a>
      </div>
    `;
        return;
    }
    ...
    container.innerHTML = itemsToShow.map(item => {
        ...
        <article class="course-card">
          <button class="favourite-btn active" 
                  onclick="toggleFavourite(event, ${item.id}, 'course')" 
          ...
          <div class="course-card-content">
            ...
            <p class="course-card-description">${item.description}</p>
            <div class="course-card-meta">
              <span class="meta-item">👤 ${item.instructor}</span>
              <span class="meta-item">⏱️ ${item.duration}</span>
            ...
            <a href="course-details.html?id=${item.id}&type=course" class="btn btn-primary">Δες περισσότερα</a>
          </div>
        </article>
      `;
        } else {
        ...
        <article class="book-card">
          <button class="favourite-btn active" 
                  onclick="toggleFavourite(event, ${item.id}, 'book')" 
                  aria-label="Αφαίρεση από αγαπημένα">
            <span class="heart heart-outline">🤍</span>
            <span class="heart heart-filled">❤️</span>
          </button>
          <img src="${item.image}" alt="${item.title}" class="book-card-image">
          <div class="book-card-content">
            <h3 class="book-card-title">${item.title}</h3>
            <p class="course-card-description">${item.author}</p>
            <p class="book-card-description">${item.description}</p>
            <div class="book-card-meta">
              <span class="meta-item">${item.type === 'book' ? '📖' : '🎥'} ${item.type === 'book' ? item.pages + ' σελ.' : item.duration}</span>
              <span class="meta-item">⭐ ${item.rating}</span>
            </div>
            <div class="tags">
              <span class="tag">${item.category}</span>
              <span class="tag">${item.type === 'book' ? 'Βιβλίο' : 'Βίντεο'}</span>
            </div>
            <a href="course-details.html?id=${item.id}&type=book" class="btn btn-primary">Δες περισσότερα</a>
          </div>
        </article>
      `;
        }
