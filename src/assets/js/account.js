    ...
    toggleButtons.forEach(button => {
        button.addEventListener('click', function () {
            ...
            if (targetInput) {
                ...
                this.setAttribute('aria-label', isPassword ? 'Απόκρυψη κωδικού' : 'Εμφάνιση κωδικού');
            }
        });
    });
}
...
function handlePasswordChange() {
    ...
    if (newPassword.length < 8) {
        showFieldError('new-password', 'Ο κωδικός πρέπει να έχει τουλάχιστον 8 χαρακτήρες');
        return;
    }

    if (newPassword !== confirmPassword) {
        showFieldError('confirm-password', 'Οι κωδικοί δεν ταιριάζουν');
        return;
    }

    if (currentPassword === newPassword) {
        showFieldError('new-password', 'Ο νέος κωδικός πρέπει να είναι διαφορετικός από τον τρέχοντα');
        return;
    }
    ...
    if (savedCards.length === 0) {
        container.innerHTML = `
            <div style="text-align: center; padding: var(--space-8); color: var(--color-text-muted);">
                <p style="font-size: var(--font-size-lg); margin-bottom: var(--space-2);">💳</p>
                <p>Δεν έχετε προσθέσει καμία κάρτα ακόμα.</p>
            </div>
        `;
        return;
    }
            ...
            <div style="display: flex; align-items: center; gap: var(--space-3);">
                ...
                <div style="flex: 1;">
                    <div style="display: flex; align-items: center; gap: var(--space-2); margin-bottom: var(--space-1);">
                        ...
                        <span style="color: var(--color-text-muted);">•••• ${card.lastFour}</span>
                    </div>
                    <div style="color: var(--color-text-dim); font-size: var(--font-size-sm);">
                        ${card.holderName} • Λήγει ${card.expiry}
                    </div>
                </div>
                ${card.isDefault ? `
...
function initAddCardForm() {
    ...
    // Only allow numbers for CVV and limit to 3 digits
...
function handleAddCard() {
    ...
    if (cardNumber.length < 15 || cardNumber.length > 16) {
        showFieldError('card-number', 'Μη έγκυρος αριθμός κάρτας');
        return;
    }

    if (!holderName) {
        showFieldError('card-holder', 'Εισάγετε το όνομα κατόχου');
        return;
    }

    // Validate name contains only letters and spaces
    if (!/^[a-zA-Z\u0391-\u03a9\u03b1-\u03c9\u03af\u03ca\u0390\u03cc\u03ac\u03ad\u03cd\u03cb\u03b0\u03ae\u03ce\s]+$/.test(holderName)) {
        showFieldError('card-holder', 'Το όνομα πρέπει να περιέχει μόνο γράμματα');
        return;
    }

    if (!/^\d{2}\/\d{2}$/.test(expiry)) {
        showFieldError('card-expiry', 'Μη έγκυρη ημερομηνία (MM/YY)');
        return;
    }
    ...
    if (year < currentYear || (year === currentYear && month < currentMonth)) {
        showFieldError('card-expiry', 'Η κάρτα έχει λήξει');
        return;
    }

    if (cvv.length !== 3) {
        showFieldError('card-cvv', 'Ο CVV πρέπει να είναι 3 ψηφία');
        return;
    }
...
function showConfirmModal(title, message, onConfirm) {
    ...
    confirmBtn.textContent = 'Αφαίρεση';
